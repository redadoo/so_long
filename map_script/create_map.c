...
/*   Updated: 2023/05/19 18:44:00 by evocatur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	spawn_env(t_game game)
{
	...
	x = 0;
	y = 0;
	test.img = mlx_xpm_file_to_image(game.mlx,MONEY,&test.size.x,&test.size.y);
	test.b_img = mlx_xpm_file_to_image(game.mlx,GRASS,&test.size.x,&test.size.y);
	while (game.map.matrix[y])
	{
		while (game.map.matrix[y][x] != '\0')
		{
